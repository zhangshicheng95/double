//发送获取的设置信息
	...
	var operatornum;			//管理员账号
	var operatorpwd;			//管理员密码
	var userout;				//退出账号&&进入维护页面
	var pwdout;					//退出密码&&进入维护页面
function message() {
	mkt = $('#store_number').val();
	syjh = $('#user_name').val();
	operator = $('#bar_name').val();
	cdkey = $('#serial_number').val();
	yys = "Accuree";
	
	if(mkt==""||syjh==""||operator==""||cdkey==""){
		alert("输入的信息不能为‘空’");
		return;
	}
	if(isNaN(mkt)){
	...
	store_message = 'store_message|{'+'"mkt":"'+mkt+'", "syjh":"'+syjh+'", '+
	'"operator":"'+operator+'", "cdkey":"'+cdkey+'", "yys":"'+yys+'}';
	alert('配置信息完成');
	jump(store_message);
	$('#store_number').val("");
	$('#user_name').val("");
...
function Machine_login() {
	operatornum = $("#machine_username").val();
	operatorpwd = $("#machine_pwd").val();
	if(operatornum==""||operatorpwd==""){
		alert("输入的信息不能为‘空’");
		return;
	}
	store_message = 'operator_login|{'+'"operator_num":"'+operatornum+'",'+'"operator_pwd":"'+operatorpwd+'"}';
	jump(store_message);
}

function Machine_out() {
	userout = $("#machine_userout").val();
	if(userout==""){
		alert("输入的信息不能为‘空’");
		return;
	}
	store_message = 'operator_ok|'+userout;
	jump(store_message);
}      
	...
	$.fn.ioskeyboard = function(options) {
		...
		this.each(function() {
			...
			if(numkeyboardcount < 2) {
				$("#message").append("<ul id='keyboard_5xbogf8c'>" +
					"<li>1</li>" +
					"<li>2</li>" +
					"<li>3</li>" +
					"<li>4</li>" +
			...
			if(numkeyboardcount < 2) {
				...
				keys.click(function(event) {
					...
					if(evebool) {
						...
						if(colorchange) {
							...
							var newbabkground = oldbabkground ;
				...
				keyboard.children("li:eq(" + 21 + ")").mousedown(function(event) {
						$(this).css({
							'top': "4.6em",
							"background": "rgba(254, 254, 254, 0.901961)",
